...
import { dropUser, logOut } from '@/store/reducers/authSlice'
...
const PersonalAccount = () => {
	...
	const [showModalConfirmDelition, setShowModalConfirmDelition] =
		useState(false)
	...
	const [
		deleteUser,
		{ isLoading, data: deleteUserData, isSuccess: isDeletionSuccess },
	] = useDeleteMutation()
	const [
		updateUserData,
		{
		...
		if (uid) {
			if (userTrips.length !== 0) {
				userTrips.map(userTrip => {
					const seats = AllTrips.filter(trip => trip._id === userTrip.tripId)[0]
						.seats
					console.log({
						id: userTrip.tripId,
						type: 'DELETE_ORDER',
						...
						availableSeats: userTrip.tripData.availableSeats,
						reservedSeats: userTrip.tripData.reservedSeats,
						amountOfTickets: userTrip.seats.length,
					})
					upDateTripInfo({
						id: userTrip.tripId,
						type: 'DELETE_ORDER',
	...
	useEffect(() => {
		if (
			isUpdateSuccess &&
			UpdateUserData?.message === 'User successfully updated'
		) {
			setAlertType('success')
		...
		if (
			(isUpdateSuccess &&
				UpdateUserData?.message ===
					'User passengers was successfully updated') ||
			'User passengers was successfully updated'
		) {
		...
		if (
			isUpdateSuccess &&
			UpdateUserData?.message === 'Passenger was successfully deleted'
		) {
			setIsEditPassengerInfo(false)
		...
		if (
			isUpdateSuccess &&
			UpdateUserData?.message === 'Password successfully updated'
		) {
			setShowModal(false)
			console.log(currentPassword)
			setAlertText('Пароль был успешно сохранен')
		...
		//  else if (
		// 	!isUpdateSuccess &&
		// 	UpdateUserData?.message === 'Incorrect password'
		// ) {
		// 	console.log('Неверный пароль')
		// 	setAlertText('Неверный пароль')
		// 	setAlertType('error')
		// }
	...
	useEffect(() => {
		if (
			...
			setUserTrips(prev => {
				let newUsersTrips = prev.filter(
					trip => trip.orderId !== deletedTrip.orderId
				)
	...
	useEffect(() => {}, [isUpdateSuccess, UpdateUserData])
