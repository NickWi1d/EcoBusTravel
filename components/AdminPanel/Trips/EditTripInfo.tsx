...
const EditTripInfo = ({
	...
	const [
		getBusInfo,
		{
			isLoading: isGetBusInfoLoading,
			isError: isGetBusInfoError,
			data: BusInfo,
			isSuccess: isGetBusInfoSuccess,
			error: BusInfoError,
		},
	] = useLazyGetBusesQuery()
	const [
		getFaultyTrip,
		{
			isLoading: isFaultyTripLoading,
			isError: isGettingFaultyTripError,
			data: FaultyTrip,
			isSuccess: isGettingFaultyTripSuccess,
			error: getingFaultyTripError,
		},
	] = useLazyGetFaultyTripQuery()

	const tripId = uuidv4()
	const [warningType, setWarningType] = useState<typeOfBus | undefined>(
		undefined
	)
	...
	useEffect(() => {
		if (isAddNewTrip) {
			...
			setAreThereAnyPassengers(
				selectedTrip.seats.filter(seat => !seat.available).length !== 0
			)
	...
	useEffect(() => {
		if (isGetBusInfoSuccess && BusInfo) {
			...
			if (!areThereAnyPassengers) {
				setSeats(
					Array.from({ length: BusInfo.bus?.type === 'L' ? 50 : 25 }, () => ({
						orderId: '',
						available: true,
						user: null,
						owner: null,
	...
	const handleDownload = async () => {
		...
		// Добавление заголовка
		doc.setFontSize(16)
		doc.text(`Список пассажиров на рейс №${selectedTrip._id}`, 105, 10, {
			align: 'center',
		})
		...
		doc.text(
			`${formatToPrintDate(selectedTrip.date)} в ${selectedTrip.startTime}`,
			15,
			50
		)
		...
		doc.text(
			`${formatToPrintDate(selectedTrip.date)} в ${selectedTrip.finishTime}`,
			120,
			50
		)
		...
		// Данные пассажиров
		const passengers = selectedTrip.seats
			.map((trip, index) => {
				return {
					seatNum: index,
					trip: trip,
			...
			.filter(trip => trip.trip.available === false)
			.map(trip => {
				return [
					`${trip.trip.owner?.surname} ${trip.trip.owner?.name} ${trip.trip.owner?.patronymic}`,
					`${trip.seatNum}`,
					`${trip.trip.owner?.birthDate}`,
					`${trip.trip.user?.phoneNumber}`,
				]
			})
			...
			<form onSubmit={EditTripInfo}>
				{areThereAnyPassengers ? (
					<>
						...
						{type !== undefined && isGetBusInfoLoading !== true ? (
							<TripPlaces
								busType={type}
								seats={seats}
								setIsShowSeatWindow={setIsShowSeatWindow}
					...
					<>
						<div className='grid grid-rows-2 grid-cols-5 gap-4 mb-[2%]'>
							...
							<FormControl
								variant='outlined'
								sx={{ minWidth: 120 }}
								error={selectedTrip.warning === false && true}
							>
								...
								>
									{selectedTrip.warning === false
										? busesList
												.filter(bus => bus.type === warningType)
												.map((bus, index) => {
													return (
														<MenuItem key={index} value={bus._id}>
															{bus.plateNumber}
														</MenuItem>
													)
												})
						...
						{isGetBusInfoLoading !== true && type !== undefined ? (
							<TripPlaces
								busType={type}
								seats={seats}
								setIsShowSeatWindow={setIsShowSeatWindow}
				...
				<div className='flex justify-end'>
					<button className='mt-3' onClick={cancelBtn}>
						Отмена
					</button>
					<Button
						type='submit'
					...
					>
						{isAddNewTrip ? 'Добавить' : 'Сохранить'}
					</Button>
				</div>
			</form>
		</div>
