...
import { useLazyLoginQuery } from '@/store/reducers/api/app';
...
const ChangePassword: FC<ChangePasswordProps> = ({ currentPassword, newPassword, newPasswordRepeat, setCurrentPassword, setNewPassword, setNewPasswordRepeat, setShowModal, UpdateUserPasswordHandler, alertText, alertType }) => {
    ...
    return (
        <form onSubmit={ChengePasswordHandler} className='flex-col items-center justify-center'>
            <h1 className='text-center'><b>Измените пароль</b></h1>
            ...
            />
            <label htmlFor="currentPassword" className={alertType === 'error' ? 'text-red-600' : ' text-black '}>Подтвержение нового пароля {alertType === 'error' && ' - пароли не совпадают'}</label>
            <input
                type="password"
            ...
            <div className='flex justify-end'>
                <button className='mt-4' onClick={(e) => CancelBtnHandler(e)}>Отмена</button>
