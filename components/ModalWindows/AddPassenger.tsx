...
const AddPassenger: FC<PassengerFormProps> = ({
    ...
    }, [passengerId, surname, name, patronymic, documentNumber, birthDate, gender])


    return (

        <form onSubmit={AddNewPassenger} className='flex-col items-center justify-center'>
            <Button variant="outlined" size="small" color="error" className='ml-[2%] mb-[2%]' onClick={()=>deletePassenger(passengerId)}>
                Удалить пасажира
            </Button>
            <PassengerForm
                surname={surname}
